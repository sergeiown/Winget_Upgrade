...
### 4. Екран налаштувань
Натисніть `F2` у будь-який момент, щоб відкрити екран налаштувань:
- **Автозапуск** - чекбокс, що створює або видаляє ярлик у папці автозавантаження Windows, тож поведінку "запускати при вході" можна вмикати й вимикати без перевстановлення.
- **Список ігнорування** - прокручуваний список усіх встановлених пакетів із чекбоксами; позначення пакета додає його точний ідентифікатор у `winget_ignore.txt`, знята позначка видаляє. Записи, додані вручну (часткові збіги типу `chrome`), не зачіпаються.
- **Мова** - перемикання між українською та англійською; застосовується миттєво, без перезапуску.
