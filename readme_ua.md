...
flowchart TD
    ...
    Updater -- "оновлення встановлено" --> Relaunch["Перезапуск оновленої програми"]
    Updater -- "немає оновлень / відмова" --> Check["Перевірка наявності winget + версії"]
    Check --> Discover["utils.js<br/>discoverUpgradablePackages"]
    Ignore[["winget_ignore.txt"]] --> Discover
    Discover --> Show["console_ui.js<br/>printDiscoveredPackages + пауза"]
    Show --> Loop["utils.js<br/>upgradePackage (по пакету)"]
    Loop --> Progress["console_ui.js<br/>прогрес-бар / ETA"]
    Loop --> Log[("winget_upgrade.log")]
    Loop --> Summary["console_ui.js<br/>printSummaryTable"]

    Settings["settings.js"]-.шляхи й аргументи.->Main
    Settings-.шляхи й аргументи.->Updater
    Settings-.шляхи й аргументи.->Discover
    Settings-.шляхи й аргументи.->Loop
```
...
При кожному запуску (але не частіше ніж раз на 24 години) програма перевіряє наявність новішого релізу на GitHub. Якщо такий є, вона запитує підтвердження перед завантаженням і тихим встановленням оновлення, після чого автоматично перезапускається. Відмова від оновлення просто пропускає цей крок і продовжує зі звичайним процесом оновлення програм.
...
3. У групі Start Menu також є ярлик прямо до списку ігнорування (`winget_ignore.txt`) для швидкого редагування.
